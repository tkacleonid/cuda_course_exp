...
#define square(i, j, k) square[(i) * P_SIZE * P_SIZE + (j) * P_SIZE + k]
...
__global__ void rhs_x_kernel(double *rho_i, double *us, double * vs, double * ws,
                             double * speed, double * qs, double * square, double * u, 
                             double * rhs, double * forcing, const int nx2, const int ny2, const int nz2,
                             const double dssp, const double xxcon2, const double xxcon3, const double xxcon4, const double xxcon5,
                             const double dx1tx1, const double dx2tx1, const double dx3tx1, const double dx4tx1, const double dx5tx1,
                             const double tx2, const double c1, const double c2, const double con43)
...
void compute_rhs()
{
    ...
    cudaMemcpy(gpuRho_i, rho_i, sizeof(double) * P_SIZE * P_SIZE * P_SIZE , cudaMemcpyHostToDevice);
    cudaMemcpy(gpuUs, us, sizeof(double) * P_SIZE * P_SIZE * P_SIZE , cudaMemcpyHostToDevice);
    cudaMemcpy(gpuVs, vs, sizeof(double) * P_SIZE * P_SIZE * P_SIZE , cudaMemcpyHostToDevice);
    cudaMemcpy(gpuWs, ws, sizeof(double) * P_SIZE * P_SIZE * P_SIZE , cudaMemcpyHostToDevice);
    cudaMemcpy(gpuSpeed, speed, sizeof(double) * P_SIZE * P_SIZE * P_SIZE , cudaMemcpyHostToDevice);
    cudaMemcpy(gpuQs, qs, sizeof(double) * P_SIZE * P_SIZE * P_SIZE , cudaMemcpyHostToDevice);
    cudaMemcpy(gpuSquare, square, sizeof(double) * P_SIZE * P_SIZE * P_SIZE , cudaMemcpyHostToDevice);
    cudaMemcpy(gpuU, u, sizeof(double) * P_SIZE * P_SIZE * P_SIZE * 5, cudaMemcpyHostToDevice);
    cudaMemcpy(gpuRhs, rhs, sizeof(double) * P_SIZE * P_SIZE * P_SIZE * 5, cudaMemcpyHostToDevice);
    cudaMemcpy(gpuForcing, forcing, sizeof(double) * P_SIZE * P_SIZE * P_SIZE * 5, cudaMemcpyHostToDevice);

    rhs_x_kernel<<<dim3(nx2, ny2/32 + 1, nz2/4 + 1), dim3(1, 32, 1)>>>(gpuRho_i, gpuUs, gpuVs, gpuWs, gpuSpeed, 
                        gpuQs, gpuSquare, gpuU, gpuRhs, gpuForcing, nx2, ny2, nz2, 
                        dssp, xxcon2, xxcon3, xxcon4, xxcon5, dx1tx1, dx2tx1, dx3tx1, dx4tx1, dx5tx1, tx2, c1, c2, con43);

    cudaMemcpy(rhs, gpuRhs, sizeof(double)*P_SIZE*P_SIZE*P_SIZE*5, cudaMemcpyDeviceToHost);
